...
EipStatus ApplicationInitialization(char * ipAddressCamera);  // my business.
...
int main( int argc, char* argv[] )
{
    ...
    if( argc != 8 )
    {
        printf( "Wrong number of command line parameters! %d instead of 8\n", argc );
        printf( "The correct command line parameters are:\n" );
        printf( "%s ipaddresshost ipaddresscamera subnetmask gateway domainname hostaddress macaddress\n", argv[0] );
        printf( "e.g.\n" );
        printf( "    %s 192.168.0.2 192.168.0.69 255.255.255.0 192.168.0.1 test.com testdevice 00:15:C5:BF:D0:87\n", argv[0] );
        ret = 1;
        goto exit;
    }
    ...
    ConfigureNetworkInterface( argv[1], argv[3], argv[4] );
    ConfigureDomainName( argv[5] );
    ConfigureHostName( argv[6] );

    if( !parse_mac( argv[7], my_mac_address ) )
    {
    ...
    if( ApplicationInitialization(argv[2]) != kEipStatusOk )
    {
